...
import { apiGetRecipeById, apiPortfolioRoute } from "../../helpers/constants";

import NavBar from "../../components/NavBar";
import RecipePortfolioCard, {type Recipe} from "../../components/recipe-components/RecipePortfolioCard";
...
const ChefPortfolioPage = () => {
  ...
  const { data: chefData, isLoading } = useQuery({
    ...
    queryFn: () => api.get(apiPortfolioRoute(id!)).then((res) => res.data.data),
  });
  ...
  const updateMutation = useMutation<AxiosResponse, Error, FormData>({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["chefPortfolio", id] });
      setIsEditing(false);
  ...
  const onSave = (formDataJson: FieldValues) => {
  ...
  // Add this next to your updateMutation
  const deleteMutation = useMutation({
    mutationFn: (recipeId: string) => api.delete(apiGetRecipeById(recipeId)),
    onSuccess: () => {
      // Refresh the portfolio data so the deleted recipe disappears
      queryClient.invalidateQueries({ queryKey: ["chefPortfolio", id] });
    },
  });
  ...
  return (
    <div className="min-h-screen bg-white">
            ...
            <div className="flex gap-3 pb-2">
              {isOwner ? (
                ...
                <>
                  <button
                    type="button"
                    className="bg-primary text-white px-8 py-3 rounded-full font-bold text-xs uppercase tracking-widest hover:scale-105 transition-transform"
                  >
                    Follow
                  </button>
                  <button
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 border-t border-border pt-8">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="flex flex-wrap gap-4">
                {chefData.chef.socials?.map((platform: string) => (
                  <button
                    key={platform}
                    type="button"
      ...
      <section className="bg-primary py-20 px-6 min-h-150">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {chefData?.recipes.map((recipe: Recipe) => (
              <RecipePortfolioCard
                recipe={recipe}
                isOwner={isOwner}
            ...
            {isOwner && (
              <Link
                to="/recipes/create"
                className="border-2 border-dashed border-white/20 rounded-4xl flex flex-col items-center justify-center p-12 text-white/40 hover:text-white hover:border-white transition-all gap-4"
              >
