...
import RecipePortfolioCard from "../../components/RecipePortfolioCard";
...
const ChefPortfolioPage = () => {
	...
	const deleteMutation = useMutation({
		mutationFn: (recipeId) => api.delete(apiGetRecipeById(recipeId)),
		onSuccess: () => {
					...
					<div className="grid grid-cols-1 lg:grid-cols-3 gap-12 border-t border-border pt-8">
						<div className="lg:col-span-2 space-y-6">
							<div className="flex gap-8 text-[10px] font-black uppercase tracking-[0.2em] text-neutral/30">
								<span>
									{chefData?.followersCount || 0} Followers
								</span>
								<span>
									Rating: {chefData?.rating || "X.X"}/5
								</span>
							</div>
			...
			<section className="bg-primary py-20 px-6 min-h-[600px]">
				<div className="max-w-7xl mx-auto">
					<div className="flex flex-wrap gap-4 mb-16">
						{["Chef's Choice", "Mains", "Pastry", "Quick"].map(
							(cat, i) => (
								<button
									key={cat}
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
						{chefData?.recipes.map((recipe) => (
							<RecipePortfolioCard recipe={recipe} isOwner={isOwner} handleDelete={handleDelete} />
						))}
