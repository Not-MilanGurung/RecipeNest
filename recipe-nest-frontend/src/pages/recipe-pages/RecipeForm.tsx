...
function RecipeForm() {
  ...
  const handleImageChange = (e : ChangeEvent<HTMLInputElement>, onChange: (file: File | null) => void) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setPreview(reader.result as string); // For the UI preview
      };
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    try {
      ...
      formData.append("steps", JSON.stringify(data.steps.map((step) => step.instruction)));
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="space-y-2">
            <label className="text-xs font-black text-primary/40 tracking-widest uppercase">
              Prep Time
            </label>
            <input
              {...register("metrics.preptime")}
              className="w-full p-4 bg-white rounded-2xl border border-border outline-none focus:ring-2 focus:ring-primary/20"
              placeholder="15 mins"
            />
          </div>
          <div className="space-y-2">
        ...
        <div className="relative group">
          <Controller
            ...
            render={({ field: { onChange } }) => (
              <label
                className={`
					h-full min-h-87.5 rounded-4xl flex flex-col items-center justify-center p-4 text-center 
                ...
                {preview ? (
                  <div className="relative w-full h-full">
                    ...
                    <div className="absolute inset-0 bg-neutral/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity rounded-[1.8rem]">
                      <span className="text-white font-bold text-sm bg-primary px-4 py-2 rounded-full">
                        Change Image
                      </span>
                    </div>
                  </div>
                  ...
                  <>
                    ...
                    <p className="text-xs text-primary/40 mt-2 px-6 italic">
            ...
            {ingredientFields.map((field, index) => (
              <div key={field.id} className="flex gap-4 items-center">
                ...
                <button
                  ...
                  className="text-white/70 hover:text-red-400 font-bold px-2"
                >
                  ✕
                </button>
              </div>
        ...
        <section className="space-y-8">
          ...
          <div className="space-y-6">
            {stepFields.map((field, index) => (
              <div
                key={field.id}
                className="flex items-start gap-8 bg-white p-8 rounded-4xl border border-border group"
              >
                <span className="text-5xl font-black text-primary/10 group-focus-within:text-primary/30 transition-colors">
                  {index + 1}
                </span>
                <textarea
                ...
                <button
                  ...
                  className="text-neutral/20 hover:text-red-500"
                >
                  Remove
                </button>
              </div>
