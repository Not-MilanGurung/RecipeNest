...
import RecipeSearchCard, { type Recipe} from "../../components/recipe-components/RecipeSearchCard";
...
function RecipesListPage() {
  ...
  const onApplyFilters = (formData: Filters) => {
    ...
    <>
      ...
      <div className="min-h-screen bg-secondary flex flex-col lg:flex-row">
        ...
        <aside className="w-full lg:w-80 p-12 bg-white border-r border-border lg:h-screen lg:sticky lg:top-0 overflow-y-auto">
          <h2 className="text-4xl font-bold mb-10 tracking-tighter text-neutral">
            Library
          </h2>

          <form onSubmit={handleSubmit(onApplyFilters)} className="space-y-12">
            <section className="space-y-4">
              <label className="text-[10px] font-black uppercase tracking-[0.2em] text-primary block">
                Search
              </label>
              ...
              <div className="space-y-3">
                {categories.map((cat) => (
                  ...
                  >
                    ...
                    />
                    <span className="text-sm font-medium text-neutral/60 group-hover:text-primary transition-colors">
                      {cat}
                    </span>
                  </label>
                ))}
        ...
        <main
          className={`grow p-8 lg:p-16 space-y-12 transition-opacity duration-300 ${isPlaceholderData ? "opacity-50" : "opacity-100"}`}
        >
            ...
            <div>
              <p className="text-neutral/40 font-bold mb-1">
                {activeFilters.search
                  ...
                  : `Showing ${data?.data.pagination.total || 0} Results`}
              </p>
          ...
          <div className="space-y-8">
            {isLoading && !data
              ? [1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-56 bg-white animate-pulse rounded-4xl border border-border"
                  />
                ))
          ...
          <div className="flex justify-center items-center gap-8 pt-12">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-neutral/40 font-bold text-xl">
                {data?.data.pagination.pages || 1}
              </span>
            </div>
            <button
              disabled={
                activeFilters.page >= (data?.data.pagination.pages || 1)
              }
              onClick={() => handlePageChange(activeFilters.page + 1)}
              className="text-[10px] font-black uppercase tracking-widest text-neutral hover:text-primary disabled:opacity-20 transition-all"
            >
