...
import { UserContext, type UserContextData } from "../../helpers/contexts";
...
const ProfilePage = () => {
  ...
  const updateMutation = useMutation<AxiosResponse, Error, FormData>({
    ...
    onSuccess: (response) => {
      ...
      setData((prev) => ({...prev, user: updatedUser} as UserContextData));
  ...
  const avatarMutation = useMutation<AxiosResponse, Error, Blob>({
    mutationFn: (file) => {
              ...
              <div className="relative group">
                ...
                >
                  <input
                    type="file"
                    className="hidden"
                    onChange={(e) => e.target.files && avatarMutation.mutate(e.target.files[0])}
                    accept="image/*"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  ...
                  <input
                    {...register("name")}
                    disabled={!isEditing}
                    className={`w-full p-5 rounded-3xl outline-none transition-all ${isEditing ? "bg-secondary ring-2 ring-primary/5" : "bg-transparent border-none"}`}
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-primary/50 ml-4">
