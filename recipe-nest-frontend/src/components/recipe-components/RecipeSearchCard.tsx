        ...
        <Link to={`/recipes/${recipe._id}`}>
          <div>
            <div className="flex gap-2 mb-3">
              ...
              {/* ADD THIS */}
              <RatingBadge average={recipe.ratingAverage} count={recipe.ratingCount} />
            </div>

            <h3 className="text-3xl font-bold mb-2 group-hover:text-primary transition-colors tracking-tight">{recipe.name}</h3>
            <p className="text-neutral/50 text-sm italic line-clamp-2 max-w-2xl leading-relaxed">"{recipe.description}"</p>
          </div>
        </Link>
          ...
          <div className="flex items-center gap-3">
            <Link to={`/chefs/${recipe.chef._id}`}>
              <div className="w-6 h-6 rounded-full bg-secondary border border-border overflow-hidden">
                <img src={recipe.chef?.avatar || `/DefaultAvatar.jpg`} alt="chef" />
              </div>
              <span className="text-[11px] font-bold uppercase tracking-widest text-neutral/60">{recipe.chef?.name}</span>
            </Link>
          </div>
        </div>
      </div>
