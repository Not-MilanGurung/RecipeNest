    ...
    <header className="sticky top-0 z-50 w-full h-20 bg-secondary text-neutral shadow-lg px-6 md:px-12 lg:px-[10%]">
      <div className="flex h-full justify-between items-center max-w-7xl mx-auto">
        ...
        <nav className="hidden md:flex h-full text-lg font-medium">
          <NavLink className={navLinkStyle} to="/recipes">
            Recipes
          </NavLink>
          {user?.role === "chef" && (
        ...
        <div className="flex items-center space-x-4">
          {user ? (
            ...
            <div className="hidden md:flex items-center space-x-4">
              ...
              <NavLink
                to="/register"
                className="text-sm font-bold uppercase tracking-widest bg-primary text-white px-6 py-2 rounded-full"
              >
                Join
              </NavLink>
            </div>
      ...
      <div
        className={`
            ...
            `}
      >
        <NavLink onClick={() => setIsMenuOpen(false)} to="/recipes" className="text-3xl font-bold">
          Recipes
        </NavLink>
        ...
        <div className="pt-8 border-t border-border flex flex-col space-y-4">
          {user ? (
            ...
            <>
              <NavLink onClick={() => setIsMenuOpen(false)} to="/login" className="text-2xl font-bold">
                Login
              </NavLink>
              <NavLink onClick={() => setIsMenuOpen(false)} to="/register" className="text-2xl font-bold text-primary">
                Register
              </NavLink>
            </>
          )}
