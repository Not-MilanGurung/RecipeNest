...
function CommentSection({ recipeId, currentUser }) {
  ...
  const createMutation = useMutation({
    mutationFn: (content) => authAPI.post(`/recipes/${recipeId}/comments`, { content }).then(res => {console.log("Creating"); return res;}),
    onSuccess: () => {
      setCommentText("");
  ...
  return (
    <div className="mt-24 space-y-12">
      <div className="flex items-center justify-between">
        <h2 className="text-4xl font-bold tracking-tight">Thoughts <span className="text-primary/20 ml-2">({comments.length})</span></h2>
      </div>
      ...
      {currentUser ? (
        <form onSubmit={handleSubmit} className="relative">
          <textarea
            ...
            className="w-full bg-white border border-border p-6 rounded-3xl min-h-[120px] outline-none focus:ring-2 focus:ring-primary/10 transition-all text-neutral"
          />
          <button 
            type="submit"
