    ...
    <div className="py-6 border-b border-border last:border-none group">
      <div className="flex justify-between items-start mb-2">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-full overflow-hidden bg-secondary">
            <img src={comment.user?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${comment.user?.name}`} alt="avatar" />
          </div>
          <div>
        ...
        {(isOwner || isAdmin) && (
          <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            {isOwner && (
              <button onClick={() => setIsEditing(!isEditing)} className="text-[10px] uppercase font-bold text-primary">Edit</button>
            )}
            <button onClick={() => onDelete(comment._id)} className="text-[10px] uppercase font-bold text-red-500">Delete</button>
          </div>
        )}
      </div>
      ...
      {isEditing ? (
        <div className="mt-2 space-y-2">
          ...
          />
          <button onClick={handleUpdate} className="px-4 py-1 bg-primary text-white text-[10px] font-bold rounded-full uppercase">Update</button>
        </div>
      ) : (
