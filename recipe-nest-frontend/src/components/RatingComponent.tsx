...
function RatingComponent({ recipeId, ratingAverage, ratingCount }: { recipeId: string, ratingAverage: number, ratingCount: number }) {
  ...
  const { mutate } = useMutation<AxiosResponse, Error, number>({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["userRating", recipeId] });
    },
  });
